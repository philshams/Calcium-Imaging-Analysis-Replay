function [stack, header] = load_tiff(tifpaths, nbplanes, nbchannels)
    % load a set of TIFF files, possibly using a GRABinfo.mat file

    % flag to permute z-planes and time axes if it is a Z-stack
    perm_z_t = false;

    % try to find metadata from GRABinfo.mat if no user inputs
    if isempty(nbplanes) && isempty(nbchannels)
        [nbchannels, nbplanes] = size_from_grabfile(fileparts(tifpaths{1}));
    end

    % try ScanImage v5 format if GRABinfo.mat did not work
    if isempty(nbplanes) && isempty(nbchannels)
        [nbchannels, nbplanes, perm_z_t] = size_from_scanimage(tifpaths{1});
    end
    ...
    if numel(tifpaths) > 1
        if numel(tifpaths) > 200
            imgs = cell(1,numel(tifpaths));
            num_iters = floor(numel(tifpaths) / 200);
            for ni = 1:num_iters
                fclose all
                imgs((ni-1)*200+1:ni*200) = cellfun(@(p) TIFFStack(p, false), tifpaths((ni-1)*200+1:ni*200), 'un', false);
            end 
                fclose all
                imgs(ni*200+1:end) = cellfun(@(p) TIFFStack(p, false), tifpaths(ni*200+1:end), 'un', false);
        else
    ...
    % get header information if needed
    if nargout > 1
        header = cellfun(@getTiffTags, imgs, 'un', false);
        [header{:}] = adjustfields(header{:});
        header = cat(2, header{:});

        % empty header used for missing frames
        header_fields = fieldnames(header);
        empty_header = [header_fields, cell(numel(header_fields), 1)]';
        empty_header = struct(empty_header{:});

        % add dummy headers for missing frames
        [~, ~, ntotal] = size(stack);
        n_extra = ntotal - numel(header);
        header(end+1:end+n_extra) = empty_header;

        header = reshape(header, nbchannels, nbplanes, []);
        if ~perm_z_t
            header = permute(header, [2, 1, 3]);
        else
            header = permute(header, [3, 1, 2]);
        end
    end
end
