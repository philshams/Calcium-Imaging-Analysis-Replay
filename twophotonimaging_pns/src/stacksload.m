function [stacks, metadata] = stacksload(stackspath, varargin)
    ...
    % modified by Philip Shamash 16.07.18
    ...
    % check if all stacks are in the same folder -- if so concatenate them below
    folder_path = {};
    for ii=1:nstacks
        curr_folder_path = fileparts(stackspath{ii});
        folder_path{ii} = curr_folder_path;
    end
    same_folder_all_files = all(cellfun(@(x) strcmp(x, folder_path{1}), folder_path));
    if same_folder_all_files
        nstacks = 1;
    end
    ...
    for ii=1:nstacks
        ...
        % all TIFF files, from the same directorys
        elseif ~isempty(regexp(stackspath{ii}, '\.tiff?', 'once')) ...
                && same_folder_all_files
            tifpaths = stackspath;
            [outputs{:, ii}] = load_tiff(tifpaths, nbplanes, nbchannels);
        
        % one TIFF file, from one of several directorys
        elseif ~isempty(regexp(stackspath{ii}, '\.tiff?', 'once'))
