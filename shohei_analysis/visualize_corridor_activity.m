...
if exist('position_response_array','var')
    ...
    position_response_array.dff = zeros(num_position_bins, length(activity_struct), length(onset.(stims{1}))-2);
    ...
    for trial = 2:length(onset.a1)-1 % skip first and last trials
        ...
        for section = 1:length(corridor_panels)
            ...
            for b = 1:bins_per_panel*corridor_panels(section)
                ...
                if isempty(curr_trial_bin_inds) || curr_trial_bin_inds(end) - curr_trial_bin_inds(1) + 1 > 50 
                    % get the speed and occupancy data
                    position_response_array.occupancy(bin,trial-1) = 0;
                    position_response_array.speed(bin,trial-1) = NaN;
                    position_response_array.photodiode(bin,trial-1) = NaN;
                    position_response_array.dff(bin,:,trial-1) = NaN;
                    continue
                else
                    position_response_array.occupancy(bin,trial-1) = curr_trial_bin_inds(end) - curr_trial_bin_inds(1) + 1;
                    position_response_array.speed(bin,trial-1) = mean(behaviour_table.speed(curr_trial_bin_inds));
                    position_response_array.photodiode(bin,trial-1) = mean(behaviour_table.photodiode(curr_trial_bin_inds));
                end
                ...
                for cell = 1:length(activity_struct)
                    ...
                    if isempty(activity_struct(cell).activity)
                        position_response_array.dff(:,cell,:) = NaN;
                        continue
                    end
                    
                    % fill the activity array corresponding to the position bin
                    position_response_array.dff(bin,cell,trial-1) = mean(activity_struct(cell).activity(curr_trial_bin_inds));
                end
...
cross_val_inds = {};
% first, do all trials
cross_val_inds{1} = 1:size(position_response_array.dff_active,3); title_modifier{1} = '';
% next  pick certain trials
% cross_val_inds{2} = 1:20; %1:2:size(position_response_array.dff_active,3); 
% title_modifier{2} = ' -- trials 1-20';
% % finally pick certain other trials
% cross_val_inds{3} = 80:98; %2:2:size(position_response_array.dff_active,3); 
% title_modifier{3} = ' -- trials 1-5';

% plot all trials, as well as a 2x cross-val
for plot_iter = 1:length(cross_val_inds)
    
% get mean responses by position
mean_dff_by_position = squeeze(nanmean(position_response_array.dff_active(:,:,cross_val_inds{plot_iter}),3))';
...
gauss_filt = gausswin(ceil(bins_per_panel / 1.5)); gauss_filt = gauss_filt / sum(gauss_filt);


occupancy = nanmean(position_response_array.occupancy(:,cross_val_inds{plot_iter}),2);
occupancy = (occupancy - nanmean(occupancy)) / nanstd(occupancy);
speed = nanmean(position_response_array.speed(:,cross_val_inds{plot_iter}),2) * 80 / 5;
