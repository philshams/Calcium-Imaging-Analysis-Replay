...
for s = 1:length(stims)
    ...
    for tp = psth_window
        ...
        curr_xyshifts = session_results.xyshifts{task_session}(:,:,curr_onset_inds + tp);
      ...
      for z_pos = 1:size(avgs,3)
          for ch = 1:size(avgs,4)
                ...
                if task_session ~= reference_session
                    avgs(:,:,z_pos,ch) = imwarp(avgs(:,:,z_pos,ch), session_results.tforms{task_session}(z_pos),'OutputView', imref2d( size(avgs(:,:,z_pos,ch)) ));
                end              
